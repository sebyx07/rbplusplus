    ...
    class MultipleFilesWriter < Base
      ...
      private
        ...
        def files_differ(file)
          ...
          return true if !File.exist?(existing)
        ...
        class FileWriter
          ...
          def initialize(node, parent)
            @node = node
            @base_name = RbPlusPlus::Utils.string_as_variable(node.qualified_name)
          ...
          protected
            ...
            def write_header
              ...
              File.open(File.join(@build_dir, @header), 'w+') do |hpp|
                ...
                hpp.puts "#include \"#{custom_name}\"" if File.exist?(File.join(@build_dir, custom_name))
            ...
            def write_source
              File.open(File.join(@build_dir, @source), 'w+') do |cpp|
                ...
                @custom_includes.each do |incl|
                  cpp.puts "#include \"#{incl}\"" unless %r{#{incl}}.match?(incl_output)
                end
                ...
                if @require_custom
                  custom_name = '_rbpp_custom.rb.hpp'
                  cpp.puts "#include \"#{custom_name}\"" if File.exist?(File.join(@build_dir, custom_name))
                end
                ...
                if @register_methods
                  ...
                  @register_methods.reverse_each do |reg|
